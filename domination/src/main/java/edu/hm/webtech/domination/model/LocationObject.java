...
public class LocationObject implements ILocationObject {
	...
	@Override
	synchronized public void setGeoCoordinates(final double longitude,
	...
	public double getDistance(final double longitude, final double latitude) {
		...
		double distance = Math.acos(Math.sin(myLatitudeRad)
				* Math.sin(latitudeRad) + Math.cos(myLatitudeRad)
				* Math.cos(latitudeRad)
				* Math.cos(longitudeRad - myLongitudeRad));

		// Multiplikation des Ergebnisses mit Äquatorradius
		return distance * EQUATOR_RADIUS;
	}
}
